...
export const WorkoutAssessment: React.FC<WorkoutAssessmentProps> = ({ onComplete, t }) => {
  ...
  const renderStep1 = () => (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500 py-4">
      <div className="space-y-2 text-center">
        ...
        <p className="text-gray-500 text-sm font-medium px-4">{t.physicalDesc}</p>
      </div>
      ...
      <div className="flex justify-center gap-2 px-4">
        {(['male', 'female', 'other'] as const).map((g) => (
          <button 
            ...
            className={`flex-1 px-4 py-3 rounded-full border-2 font-black uppercase tracking-widest text-[9px] transition-all ${profile.gender === g ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20 text-blue-600 shadow-md' : 'border-gray-100 dark:border-gray-800 text-gray-400'}`}
          >
            {t[g] || g}
          </button>
        ))}
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500 py-4 flex flex-col">
      ...
      <div className="space-y-4 px-8 mt-4">
        ...
        <div className="flex overflow-x-auto no-scrollbar gap-2 justify-start sm:justify-center px-2 py-2">
      ...
      <div className="px-6">
        <div className="bg-gray-50 dark:bg-gray-900 p-8 rounded-[48px] border border-gray-100 dark:border-gray-800 space-y-8 shadow-inner max-w-md mx-auto">
            <div className="flex items-center justify-between">
                ...
                <button 
                    onClick={() => setProfile({ ...profile, hasInjury: !profile.hasInjury })}
                    className={`w-16 h-9 rounded-full p-1.5 transition-colors ${profile.hasInjury ? 'bg-red-500' : 'bg-gray-300 dark:bg-gray-700'}`}
                >
                    <div className={`w-6 h-6 bg-white rounded-full shadow-md transition-transform ${profile.hasInjury ? 'translate-x-7' : ''}`}></div>
                </button>
            </div>

            {profile.hasInjury && (
                <textarea 
                    className="w-full bg-white dark:bg-gray-800 rounded-[32px] p-6 text-sm border-none ring-2 ring-red-100 dark:ring-red-900/30 focus:ring-4 focus:ring-red-500/20 dark:text-white shadow-inner animate-in fade-in zoom-in duration-300 min-h-[100px]"
                    placeholder={t.injuryPlaceholder}
                    value={profile.injuryDetail || ''}
                    onChange={(e) => setProfile({ ...profile, injuryDetail: e.target.value })}
                />
            )}
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-white dark:bg-black flex flex-col transition-colors duration-500 overflow-hidden select-none">
